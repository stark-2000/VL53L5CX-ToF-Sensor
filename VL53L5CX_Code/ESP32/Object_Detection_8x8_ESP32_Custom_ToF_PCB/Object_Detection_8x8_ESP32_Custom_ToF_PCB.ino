//Object Detection using VL53L5CX custom PCB interfaced with ESP32 Board - Measures distance of object in each 64 zones (8*8 Resolution)
...
void setup()
{
  ...
  myImager.setResolution(8 * 8); //Enable all 16 pads - if needed to reduce FoV, can try 4*4 for 16 pads
      ...
      */
      ...
      for (int y = 0; y <= imageWidth * (imageWidth - 1); y += imageWidth) 
      {
        ...
        //Running this for loop 8 times enable getting values from 8 different Zone ID
        {
          ...
          zid = x+y;
          Serial.print(zid); //Adding X + Y gives the Zone ID
          Serial.print(" , Distance of Object from Sensor: ");
          dis = measurementData.distance_mm[x + y]; //Calculating distance of a Zone ID
          Serial.println(dis); 
        }
